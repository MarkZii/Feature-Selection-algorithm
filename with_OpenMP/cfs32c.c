...
extern void calcoloMediaGruppoAssembly(params *input, int j, unsigned long n0, unsigned long n1, type* differenzaMedia);
extern void calcoloPearsonCoefficient2(params *input, int indiceColonna1, int indiceColonna2, type* sommaX, type* sommaY, type* sommaXY, type* sommaX2, type* sommaY2);
extern void calcoloDeviazioneStandard2(params *input, int j, type* somma_sx, type* somma_dx);
//extern void sqrtAssembly(params* input, type* risultato) //non la abbiamo usata perchè non ne valeva la pena, visto che non cambiano i tempi con la funzione di math e visto che il passaggio del risultato per riferimento crea un overhead di tempo
...
void inizializzazione_struttura_di_supporto(params *input)  //viene richiamata nella funzione CorrelationFeatureSelection(...)
{
	...
	for(int i=0;i<input->d;i++)
	{
		...
		vettore_somme[i].flag_valori = (int*) calloc(sizeof(int), input->d);
		/*vettore_somme[i].xy = (type*) malloc(input->d * sizeof(type));			//allocco il vettore di somme che coinvolgono la colonna x
		vettore_somme[i].flag_valori = (int*)malloc(input->d * sizeof(int));	//allocco il vettore di somme che coinvolgono la colonna x
...
/*type calcoloDeviazioneStandard(params *input, int j)
{
	/*
	// Inizializzazione dei dati
	type media = 0.0;
	calcoloMediaAssembly(input, j,&media);

	type risultato;
	calcoloSommatoriaQuadrato(input, media, j, &risultato);
	type deviazioneStandard = sqrt((1.0 / (input->N - 1)) * risultato); // Valore atteso
	...
	/*for (int i = 0; i < input->N; i++)
	{
		type xi = input->ds[(j * input->N) + i]; // mi sto spostanso sui valori della colonna j-esima
		somma_sx += xi*xi;
		somma_dx += xi;
	}

	type numeratore = somma_sx - (somma_dx * somma_dx/n);
	return sqrt(numeratore/(n-1));
}*/
...
{
	/*// Calcolo media dei valori delle colonne di indice "indiceColonna1" e "indiceColonna2"
	type mediaColonna1 = 0.0;
	type mediaColonna2 = 0.0;
	calcoloMediaAssembly(input, indiceColonna1,&mediaColonna1);
	calcoloMediaAssembly(input, indiceColonna2,&mediaColonna2);;

    //calcolo numeratore 
    //type numeratore = calcoloSommatoria(input, mediaColonna1, mediaColonna2, indiceColonna1, indiceColonna2);
	type risultato;
	calcoloSommatoria(input, mediaColonna1, mediaColonna2, indiceColonna1, indiceColonna2, &risultato);

	//printf("MediaColonna1: %lf \n", mediaColonna1);
	//printf("MediaColonna2: %lf \n", mediaColonna2);

	type risultato1;
	type risultato2;
	//calcolo denominatore
	calcoloSommatoriaQuadrato(input, mediaColonna1, indiceColonna1, &risultato1);
	calcoloSommatoriaQuadrato(input, mediaColonna2, indiceColonna2, &risultato2);

    
    type denominatore = sqrt(risultato1 * risultato2);
	//printf("SOMMATORIA2: %lf \n", sommatoria);
    //restituzione valori
    return risultato/denominatore;*/
	...
	for (int i = 0; i < input->N; i++) {
		type xi = input->ds[(indiceColonna1 * input->N) + i]; // mi sto spostanso sui valori della colonna j-esima
		type yi = input->ds[(indiceColonna2 * input->N) + i]; // mi sto spostanso sui valori della colonna j-esima
		somma_Y += yi;
		somma_X += xi;
		somma_XY += xi*yi;
		somma_X2 += xi*xi;
	...
	if(vettore_somme[indiceColonna1].flag_valori[indiceColonna2]!=0 && vettore_somme[indiceColonna1].flag!=0 && vettore_somme[indiceColonna2].flag!=0)
	{
		...
		somma_XY = vettore_somme[indiceColonna1].xy[indiceColonna2];
	}else{
		calcoloPearsonCoefficient2(input, indiceColonna1, indiceColonna2, &somma_X, &somma_Y, &somma_XY, &somma_X2, &somma_Y2);
		...
		vettore_somme[indiceColonna1].xy[indiceColonna2]=somma_XY;
		vettore_somme[indiceColonna2].xy[indiceColonna1]=somma_XY;
		//aggironiamo le flag
...
type calcoloScore(int indice_feature_corrente, int indice_colonna, params* input)
{
	...
	#pragma omp parallel for reduction(+:rff_assoluto)
...
type calcoloScore(int indice_feature_corrente, int indice_colonna, params* input)
{
    ...
    //calcoliamo il point biserial correlation coefficient
    type rcf_assoluto=0.0;
	int i;	//non lo inizializziamo nel for perchè l'indice "i" va condiviso con il for dei residui
	//visto che applichiamo il loop unrolling, decidiamo di far partire i dal fattore di unroll-1, così che gli indici dentro il for siano da i a i-unrollFactor. Questa scelta implica che nel ciclo dei residui dobbiamo fare i=(i-unrollFactor)+1. Se avessimo fatto partire il primo ciclo da i=unrollFactor, allora avremmo dovuto mettere i<=k, per prendere anche il caso di k%unrollFactor==0.
    for(i=unrollFactor-1;i<k;i+=unrollFactor) {  //calcoliamo la media dei valori assoluti
        rcf_assoluto+=valoreAssoluto(pointBiserialCorrelationCoefficient(input, input->out[i-3]));
		rcf_assoluto+=valoreAssoluto(pointBiserialCorrelationCoefficient(input, input->out[i-2]));
		rcf_assoluto+=valoreAssoluto(pointBiserialCorrelationCoefficient(input, input->out[i-1]));
		rcf_assoluto+=valoreAssoluto(pointBiserialCorrelationCoefficient(input, input->out[i]));   //ci fermiamo a 3 perchè l'unrollFactor è pari a 4
	}

	//ciclo residui
	for(i=(i-unrollFactor)+1;i<k;i++) {  //calcoliamo la media dei valori assoluti
        rcf_assoluto+=valoreAssoluto(pointBiserialCorrelationCoefficient(input, input->out[i]));
	}
    type numeratore=rcf_assoluto;  //semplifichiamo il fatto che la media sia divisa per k, e si moltiplica per k
    ...
    //calcoliamo il coefficiente di correlazione di pearson tra la colonna corrente e le k-1 colonne già presenti
    type rff_assoluto=0.0; 
	for(i=unrollFactor-1;i<k-1;i+=unrollFactor){//calcoliamo la media dei valori assoluti (a sto giro i si ferma prima perchè non vogliamo calcolare il coefficiente di pearson tra la colonna appena inserita e se stessa, ovviamente la colonna appena inserita è in coda)
		rff_assoluto+=valoreAssoluto(calcoloPearsonCoefficient(input, input->out[i-3], input->out[indice_feature_corrente]));
		rff_assoluto+=valoreAssoluto(calcoloPearsonCoefficient(input, input->out[i-2], input->out[indice_feature_corrente]));
		rff_assoluto+=valoreAssoluto(calcoloPearsonCoefficient(input, input->out[i-1], input->out[indice_feature_corrente]));
		rff_assoluto+=valoreAssoluto(calcoloPearsonCoefficient(input, input->out[i], input->out[indice_feature_corrente]));
	}

	//ciclo residui
	for(i=(i-unrollFactor)+1;i<k-1;i++){//calcoliamo la media dei valori assoluti (a sto giro i si ferma prima perchè non vogliamo calcolare il coefficiente di pearson tra la colonna appena inserita e se stessa, ovviamente la colonna appena inserita è in coda)
		rff_assoluto+=valoreAssoluto(calcoloPearsonCoefficient(input, input->out[i], input->out[indice_feature_corrente]));
	}
